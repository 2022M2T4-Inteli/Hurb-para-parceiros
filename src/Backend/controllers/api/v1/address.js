...
router.post("/create", auth, hasMinimumPartnerRole, (req, res) => {
    ...
    Database.open(__dirname + '../../../../database/database.db').then(async (db) => {
        ...
        const address = {
            create: await db.run(`INSERT INTO Endereco("id_do_estabelecimento","tipo_do_logradouro","logradouro","numero","complemento","bairro","cidade","estado","cep") VALUES(${organization_id}, "${type_of_logradouro}", "${logradouro}", "${number}", "${complement}", "${neighborhood}", "${city}", "${state}", "${cep}")`),
